...
const { editBackgroundV1 } = require("../../../lib/scrapper-converter.js") 
module.exports = async (content, metadata = { color: "", apikey: "" }, options = { version: "v1" }) => {
    const results = { 
    ...
    switch (options?.version) {
        case "v1":
            return await editBackgroundV1(content, metadata).then(({ status, buffer, base64, message }) => {
                if (status) {
