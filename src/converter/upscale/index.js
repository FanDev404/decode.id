const { upscaleV1 } = require("../../../lib/scrapper-converter.js") 
const { isUrl, isBase64, isFiles } = require("../../../lib/function.js")
module.exports = async (content, size = 2, anime = false, options = { version: "v1" }) => {
    const results = { 
    ...
    switch (options?.version) {
        case "v1":
            return await upscaleV1(content, size, anime).then(({ status, data, message }) => {
                if (status) {
                    results.data = data
                    results.status = true
                } else {
