    ...
    return await youtubeDownlad.getInfo(url.trim()).then(async ({ player_response }) => {
        return await axios.get("https://api.firda.uz/youtube?url=" + url).then(async ({ status, data }) => {
            if (status == 200) {
                ...
                const download_mp3 = await fetchBuffer(data.Audio[1].url)
                const search = (quality) => data.Video.filter((x) => x.quality == quality) 
                if (search("1080p").length > 1) {
                    const download_mp4 = await fetchBuffer(search("1080p")[1].url)
                    if (!download_mp4.status) {
                        results.message = download_mp4.message
                        return results
                    }
                    results.data["download_1080p"] = {
                        "url": search("1080p")[1].url, 
                        "buffer": download_mp4.buffer, 
                        "base64": download_mp4.buffer.toString("base64")
                    }
                }
                if (search("720p").length > 1) {
                    const download_mp4 = await fetchBuffer(search("720p")[1].url)
                    if (!download_mp4.status) {
                        results.message = download_mp4.message
                        return results
                    }
                    results.data["download_720p"] = {
                        "url": search("720p")[1].url, 
                        "buffer": download_mp4.buffer, 
                        "base64": download_mp4.buffer.toString("base64")
                    }
                }
                if (search("480p").length > 1) {
                    const download_mp4 = await fetchBuffer(search("480p")[1].url)
                    if (!download_mp4.status) {
                        results.message = download_mp4.message
                        return results
                    }
                    results.data["download_480p"] = {
                        "url": search("480p")[1].url, 
                        "buffer": download_mp4.buffer, 
                        "base64": download_mp4.buffer.toString("base64")
                    }
                }
                if (search("360p").length > 1) {
                    const download_mp4 = await fetchBuffer(search("360p")[1].url)
                    if (!download_mp4.status) {
                        results.message = download_mp4.message
                        return results
                    }
                    results.data["download_360p"] = {
                        "url": search("360p")[1].url, 
                        "buffer": download_mp4.buffer, 
                        "base64": download_mp4.buffer.toString("base64")
                    }
                }
                if (search("240p").length > 1) {
                    const download_mp4 = await fetchBuffer(search("240p")[1].url)
                    if (!download_mp4.status) {
                        results.message = download_mp4.message
                        return results
                    }
                    results.data["download_240p"] = {
                        "url": search("240p")[1].url, 
                        "buffer": download_mp4.buffer, 
                        "base64": download_mp4.buffer.toString("base64")
                    }
                }
                if (search("144p").length > 1) {
                    const download_mp4 = await fetchBuffer(search("144p")[1].url)
                    if (!download_mp4.status) {
                        results.message = download_mp4.message
                        return results
                    }
                    results.data["download_144p"] = {
                        "url": search("144p")[1].url, 
                        "buffer": download_mp4.buffer, 
                        "base64": download_mp4.buffer.toString("base64")
                    }
                }
                if (!download_mp3.status) {
                    results.message = download_mp3.message
                    return results
                }
                results.data["download_mp3"] = {
                    "url": data.Audio[1].url, 
                    "buffer": download_mp3.buffer, 
                    "base64": download_mp3.buffer.toString("base64")
                }
                results.status = true
            } else {
...
const tiktokDLV4 = async (url = "") => {
    const results = { status: false, data: {}, message: "" }
    return await axios.get("https://api.nyxs.pw/dl/tiktok?url=" + url).then(({ status, data }) => {
        if (status == 200 && (data.status == true || data.status == "true")) {
            results.data["details"] = {
                "thumbnail": data.result.thumbnail,
                "caption": data.result.caption
            }
            if (data.result?.type == "videos") {
                results.data["download"] = {
                    "type": "video", 
                    "video": data.result?.video_sd, 
                    "audio": data.result?.audio, 
                }
            } else if (data.result?.type == "images") {
                results.data["download"] = {
                    "type": "image", 
                    "image": data.result?.images, 
                    "audio": data.result?.audio, 
                }
            }
            results.status = true
