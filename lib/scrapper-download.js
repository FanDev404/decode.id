    ...
    return await youtubeDownlad.getInfo(url.trim()).then(async ({ player_response }) => {
        return await axios.get("https://api.firda.uz/youtube?url=" + url).then(async ({ status, data }) => {
            if (status == 200) {
                ...
                for (const x of data.Vidio.filter((x) => parseInt(x.quality) < 1000)) {
                    const download_mp4 = await fetchBuffer(x.url)
                    if (download_mp4.status) {
                        if (!Object.keys(results.data).includes(`download_${x.quality}`)) results.data[`download_${x.quality}`] = {
                            "url": x.url, 
