...
const youtubeDLV2 = async (url = "") => {
    const results = { status: false, data: {}, message: "" }
    return await axios.get("https://api.nyxs.pw/dl/yt?url=" + url).then(({ status, data }) => {
        if (status == 200 && (data.status == true || data.status == "true") && Object.keys(data.result.data.hd).includes("url")) {
            results.data["title"] = data.result.title
            results.data["video"] = {
                "quality": data.result.data.hd.quality, 
                "size": data.result.data.hd.size, 
                "url": data.result.data.hd.url, 
            }
            results.data["audio"] = {
                "quality": data.result.data.mp3.quality, 
                "size": data.result.data.mp3.size, 
                "url": data.result.data.mp3.url, 
            }
            results.status = true
        } else if (status == 200 && (data.status == true || data.status == "true")) {
