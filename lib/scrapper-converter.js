...
const quickChatToWebpV1 = async (name, text, profile_URL) => {
    const results = { status: false, data: {}, message: "" }
    const options = {
        "type": "quote", 
        "format": "png", 
    ...
    return await axios.post("https://bot.lyo.su/quote/generate", options, { 
        "headers": { 
            "Content-Type": "application/json" 
        }
    }).then(async (response) => {
        if (response.status == 200) {
            return await imageToWebpV1(Buffer.alloc(response.data.result.image.length, response.data.result.image, "base64")).then(({ status, buffer, base64, message }) => {
                if (status) {
