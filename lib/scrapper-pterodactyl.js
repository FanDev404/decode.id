...
const createUsersV1 = async (url, options = { application_key: "", name: "", password: "", email: "", isAdmins: false }) => {
    const results = { status: false, data: "", message: "" }
    if (!options?.application_key || options?.application_key && !options?.application_key.startsWith("ptla_")) {
        results.message = "Invalid key"
        return results
    ...
    const usersName = (options?.name? options.name : randomCode(1)).toString().trim()
    ...
    const optionsAccount = JSON.stringify({
        "email": usersEmail,
        "username": usersName, 
        "first_name": usersPassword, 
        "last_name": usersPassword, 
        "language": "en", 
...
const createServersV1 = async (url, options = { application_key: "", name: "", password: "", email: "", isAdmins: false, egg: 16, locations: 1, create: "1GB" }) => {
    const results = { status: false, data: {}, message: "" }
    if (!options?.application_key || options?.application_key && !options?.application_key.startsWith("ptla_")) {
        results.message = "Invalid key"
        return results
    ...
    if (options?.create && !isNaN(parseInt(options?.create)) && options?.create.trim().toUpperCase().endsWith("GB")) {
        var memory = Number(1024 * parseInt(options?.create))
        var cpu = Number(((parseInt(options?.create) - 1) * 20) + 30)
        var disk = Number(1024 * parseInt(options?.create))
    } else {
        var memory = 0
        var cpu = 0
        var disk = 0
    ...
    if (response1.data.filter((x) => (x.username == usersName || x.email == usersEmail)).length > 0) {
        ...
        return await axios.post(applicationServers, optionsServer, { 
            "headers": {
                "Accept": "application/json",
                "Content-Type": "application/json",
                "Authorization": "Bearer " + options.application_key
            ...
            if (status == 201) {
                ...
                results.data["servers"] = data.attributes
                results.message = "Success create servers"
                results.status = true
            } else {
                results.message = "Error code3 status: " + status
            }
            return results
        }).catch(async ({ message }) => {
            const locations = await getLocationsV1(applicationNests, { "application_key": options.application_key, "id": options?.locations? options?.locations : 1 })
            if (locations?.status == false) {
                results.message = locations.message
            } else {
                results.message = "Error : " + message
            }
            return results
            ...
            if (response3?.status == 201) {
                ...
                return await axios.post(applicationServers, optionsServer, { 
                    ...
                    if (status == 201) {
                        ...
                        results.message = "Error code4 status: " + status
                    }
                    return results
                }).catch(async ({ message }) => {
                    const locations = await getLocationsV1(applicationNests, { "application_key": options.application_key, "id": options?.locations? options?.locations : 1 })
...
const updateServersV1 = async (url, options = { application_key: "", id: 1, update: "10GB" }) => {
    const results = { status: false, data: "", message: "" }
    if (!options?.application_key || options?.application_key && !options?.application_key.startsWith("ptla_")) {
        results.message = "Invalid key"
        return results
