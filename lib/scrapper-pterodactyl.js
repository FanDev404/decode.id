...
const createUsersV1 = async (url, options = { application_key: "", name: "", password: "", email: "", isAdmins: false }) => {
    ...
    const optionsAccount = JSON.stringify({
        "email": usersEmail,
        "username": usersName, 
        "first_name": usersName + "@" + usersPassword, 
...
const createServersV1 = async (url, options = { application_key: "", name: "", password: "", email: "", isAdmins: false, id: 1, egg: 16, locations: 1, create: "1GB" }) => {
    ...
    /*if (options?.create && !isNaN(parseInt(options?.create)) && options?.create.trim().toUpperCase() == "1GB") {
        var memory = 1024
        var cpu = 30
        var disk = 1024
    } else if (options?.create && !isNaN(parseInt(options?.create)) && options?.create.trim().toUpperCase() == "2GB") {
        var memory = 2048
        var cpu = 50
        var disk = 2048
    } else if (options?.create && !isNaN(parseInt(options?.create)) && options?.create.trim().toUpperCase() == "3GB") {
        var memory = 3072
        var cpu = 70
        var disk = 3072
    } else if (options?.create && !isNaN(parseInt(options?.create)) && options?.create.trim().toUpperCase() == "4GB") {
        var memory = 3072
        var cpu = 70
        var disk = 3072
*/
