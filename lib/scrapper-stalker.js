...
const gameStalkerV1 = async (game, users_id, server_id) => {
    ...
    if (game.toLowerCase() == "free fire") {
        ...
        return await axios.get("https://api.isan.eu.org/nickname/ff?" + stringify({ "id": users_id })).then(({ status, data }) => {
        ...
        const body = stringify({ 
            ...
            "user.userId": users_id, 
            "user.zoneId": server_id, 
            "voucherTypeName": "GENSHIN_IMPACT", 
            "shopLang": "id_ID"
        })
        return await axios.post("https://order-sg.codashop.com/initPayment.action", body, {
            "Content-Type": "application/x-www-form-urlencoded",
        }).then(({ status, data }) => {
            if (status == 200 && data.success) {
                results.data["game"] = game.toLowerCase()
                results.data["name"] = data.confirmationFields.username.split("+").join(" ").trim()
                results.data["id"] = users_id
                results.data["zone"] = server_id
                results.status = true
            } else if (status == 200) {
                results.message = "Nickname not found"
            } else {
                results.message = "Failed status code " + status
            }
            return results
        ...
        return await axios.get("https://api.isan.eu.org/nickname/pb?" + stringify({ "id": users_id})).then(({ status, data }) => {
            if (status == 200 && data.success) {
                results.data["game"] = game.toLowerCase()
                results.data["name"] = data.name
                results.data["id"] = users_id
        ...
        return await axios.get("https://api.isan.eu.org/nickname/sus?" + stringify({ "id": users_id})).then(({ status, data }) => {
            if (status == 200 && data.success) {
                results.data["game"] = game.toLowerCase()
                results.data["name"] = data.name
                results.data["id"] = users_id
        ...
        return await axios.get("https://api.isan.eu.org/nickname/aov?" + stringify({ "id": users_id })).then(({ status, data }) => {
            if (status == 200 && data.success) {
                results.data["game"] = game.toLowerCase()
                results.data["name"] = data.name
                results.data["id"] = users_id
        ...
        return await axios.get("https://api.isan.eu.org/nickname/ml?" + stringify({ "id": users_id, "zone": server_id })).then(({ status, data }) => {
            if (status == 200 && data.success) {
                results.data["game"] = game.toLowerCase()
                results.data["name"] = data.name
                results.data["id"] = users_id
